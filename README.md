...
将数据集放置在 `data/` 目录下，或通过软链接指向数据集路径。
...
# 下载 Talk2DINO projector 权重
# TODO: 添加权重下载链接
# wget <projector-weights-url> -O weights/vitb_mlp_infonce_flickr8k.pth

# 下载 ProtoWhisper adapter 权重
# TODO: 添加权重下载链接
# wget <adapter-weights-url> -O weights/protowhisper_adapter.pth
```
...
更多可视化结果和分析请参见：
- [技术报告](report/main.pdf) 第 6 页
- [实验文档](docs/sam3_adapter_research_report.md)
...
├── scripts/
│   ├── run_extract_coco.sh            # COCO 特征提取
│   ├── run_extract_flickr8k.sh        # Flickr8k 特征提取
│   ├── run_train_coco.sh              # COCO Projector 训练
│   ├── run_train_flickr8k.sh          # Flickr8k Projector 训练
│   ├── run_train_prototype_adapter_flickr8k.sh  # Adapter 训练
│   ├── run_infer.sh                   # Baseline 评测
│   └── run_infer_adapter.sh           # Adapter 评测
├── src/
│   ├── model.py                       # Talk2DINO projector 实现
│   ├── prototype_adapter.py           # ProtoWhisper adapter 实现
│   ├── local_adapter.py               # Dense local adapter (消融实验)
│   ├── logit_adapter.py               # Logit residual adapter (消融实验)
...
│       │   ├── dinotext.py            # 核心模型（集成 projector + adapter）
│       │   ├── masker.py              # Mask 生成模块
...
│       └── utils/                     # 工具函数
├── aux/
│   ├── visualize_single_image.py      # 单图分割可视化
│   ├── select_qualitative_examples.py # 定性样例筛选与对比
│   ├── visualize_dino_features.py     # DINO 特征可视化
│   └── sam3_img.py                    # SAM3 mask 可视化
├── docs/
│   ├── sam3_adapter_research_report.md      # 实验报告
│   └── sam_protodino_server_artifacts.md    # 服务器权重说明
├── report/
│   └── main.pdf                       # 技术报告
├── train.py                           # Talk2DINO Projector 训练入口
├── train_prototype_adapter.py         # ProtoWhisper Adapter 训练入口
├── train_local_adapter.py             # Dense local adapter 训练（消融）
├── train_logit_adapter.py             # Logit residual adapter 训练（消融）
...
使用 `aux/visualize_single_image.py` 对单张图像进行开放词汇分割可视化：

**Baseline（仅 projector）：**
```bash
python aux/visualize_single_image.py \
  --image /path/to/image.jpg \
  --projector-checkpoint weights/vitb_mlp_infonce_flickr8k.pth \
  --dataset voc \
  --output-dir outputs/visualization
```

**ProtoWhisper（projector + adapter）：**
```bash
python aux/visualize_single_image.py \
  --image /path/to/image.jpg \
  --projector-checkpoint weights/vitb_mlp_infonce_flickr8k.pth \
  --adapter-checkpoint weights/protowhisper_adapter.pth \
  --adapter-alpha 0.15 \
  --dataset voc \
  --mode slide \
  --affinity config \
...
python aux/visualize_single_image.py \
  ...
  --adapter-checkpoint weights/protowhisper_adapter.pth \
  --adapter-alpha 0.15 \
  --classes dog person grass sky building \
  --with-background \
  --bg-thresh 0.55 \
  --output-dir outputs/visualization
```
...
**输出文件：**
- `*_overlay.png`: 原图叠加分割结果
- `*_color.png`: 纯彩色语义 mask
- `*_mask.png`: 类别 index mask
- `*_scores.pt`: 每类的 score map
- `*_meta.json`: 元信息
